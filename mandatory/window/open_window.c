...
/*   Updated: 2023/01/14 13:31:42 by mcloarec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	color_background(t_data *data)
{
	...
	while (y < (SCREEN_HEIGHT / 2))
	{
		...
		{
			my_mlx_pixel_put(&data->display, x, y, data->id.ceiling); // rose : 0x00D995A0
			x++;
		}
		y++;
	}
	while (y < SCREEN_HEIGHT)
	{
		...
		while (x <= SCREEN_WIDTH)
		{
			my_mlx_pixel_put(&data->display, x, y, data->id.floor);// jaune : 0x00FFF973
			x++;
		}
		y++;
	}
...
void	ft_open_window(t_data *data)
{
	...
	data->display.addr = mlx_get_data_addr(data->display.img, &data->display.bpp,
		&data->display.l_length, &data->display.endian);
